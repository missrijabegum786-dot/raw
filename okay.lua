function SearchWrite(Search, Write, Type) gg.clearResults() gg.setVisible(false) gg.searchNumber(Search[1][1], Type) local count = gg.getResultCount() local result = gg.getResults(count) gg.clearResults() local data = {} local base = Search[1][2] if (count > 0) then for i, v in ipairs(result) do v.isUseful = true end for k=2, #Search do local tmp = {} local offset = Search[k][2] - base local num = Search[k][1] for i, v in ipairs(result) do tmp[#tmp+1] = {} tmp[#tmp].address = v.address + offset tmp[#tmp].flags = v.flags end tmp = gg.getValues(tmp) for i, v in ipairs(tmp) do if ( tostring(v.value) ~= tostring(num) ) then result[i].isUseful = false end end end for i, v in ipairs(result) do if (v.isUseful) then data[#data+1] = v.address end end if (#data > 0) then gg.toast("พบ"..#data.."พบข้อมูล") local t = {} local base = Search[1][2] for i=1, #data do for k, w in ipairs(Write) do offset = w[2] - base t[#t+1] = {} t[#t].address = data[i] + offset t[#t].flags = Type t[#t].value = w[1] if (w[3] == true) then local item = {} item[#item+1] = t[#t] item[#item].freeze = true gg.addListItems(item) end end end gg.setValues(t) else gg.toast("ไม่พบ", false) return false end else gg.toast("ไม่พบข้อมูล") return false end end
...
function B()
SN = gg.choice({ 
  "رقم 1 تفعيل فاكس🥶🥶كراش",
  "رقم 2 تفعيل 💥 حماية لوبي",
  "",
  "❌️Exit❌️"
  }, nil,"")
  if SN == 1 then B1() end
...
--如果减伤就把所有200改成150或100
gg.setRanges(32)
local dataType = 16--类型 
local tb1 = {{23.0,-8}, {25.0,-4}, {30.5,0}, } 	
local tb2 = {{200,0}, } 	
 SearchWrite(tb1,tb2, dataType)--30.5脖子
TB=200
gg.setRanges(32)
local dataType = 16--类型
local tb1 = {{23.0,0}, {25.0,4}, {200.0,8}, } 	
local tb2 = {{TB,0}, } 	
 SearchWrite(tb1,tb2, dataType)--23头部
SX=200
gg.setRanges(32)
local dataType = 16--类型 
local tb1 = {{TB,-4}, {25.0,0}, {200.0,4}, } 	
local tb2 = {{SX,0}, } 	
 SearchWrite(tb1,tb2, dataType)--25身体@MAX MR700
gg.toast("تم تفعيل🥶🥶 هيد شوت ماكس")
end
...
local ttt = S_Pointer(t, tt, true)
gg.addListItems({{address = ttt, flags = 4, value = 4096, freeze = true}})
gg.toast("تم تفعيل🥶🥶 فاكس كراش 🥶 ماكس")  
end
...
gg.clearResults()
gg.toast("تم تفعيل🥶🥶 حماية لوبي ماكس")
end
